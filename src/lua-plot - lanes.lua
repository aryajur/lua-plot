...
--lanes = lanes.configure({required = "iuplua"})
...
local plotthreadfunc = function()
	...
	print("Going into iup main loop")
...
local plotthread = lanes.gen("*",plotthreadfunc)()
local exitProg

while plotthread.status ~= "running" do
	if plotthread.status == "killed" or plotthread.status == "error" or plotthread.status == "cancelled" then
		exitProg = true
		break
	end
end
...
--	end
	while i <= #createdPlots do
		...
		if not plots[createdPlots[i]] then
			...
			linda:send("plotthread",sendMsg)
			local key,val = linda:receive(0.1,"lua-plot")
			if key then
				if val and val[1] == "ACKNOWLEDGE" then
...
function plot (tbl)
	...
	linda:send("plotthread",sendMsg)
	local key,val = linda:receive(0.1,"lua-plot")
	print("Message from plot server:",key,val)
	if not key then
		return nil, "No Acknowledgement from plot server"
	end
	sendMsg = val
